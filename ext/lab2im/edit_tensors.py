...
def blurring_sigma_for_downsampling(current_res, downsample_res, mult_coef=None, thickness=None):
    ...
    if not tf.is_tensor(downsample_res):
        ...
        if thickness is not None:
            down_res = KL.Lambda(lambda x: tf.math.minimum(
                x[0], x[1]))([downsample_res, thickness])
        else:
...
def gaussian_kernel(sigma, max_sigma=None, blur_range=None, separable=True):
    ...
    if blur_range is not None:
        if blur_range != 1:
            sigma_tens = sigma_tens * \
                tf.random.uniform(tf.shape(sigma_tens),
                                  minval=1 / blur_range, maxval=blur_range)
    ...
    if separable:
        ...
        # build meshgrid
        mesh = [tf.cast(f, 'float32')
                for f in volshape_to_meshgrid(windowsize, indexing='ij')]
        diff = tf.stack([mesh[f] - (windowsize[f] - 1) /
                        2 for f in range(len(windowsize))], axis=-1)
...
def sobel_kernels(n_dims):
    ...
    for dim in range(n_dims):
        ...
        for axis in range(n_dims):
            ...
            sublist_kernels.append(tf.expand_dims(
                tf.expand_dims(kernel, -1), -1))
...
def unit_kernel(dist_threshold, n_dims, max_dist_threshold=None):
    ...
    # get batchsize
    batchsize = None if shape[0] is not None else tf.split(
        tf.shape(dist_threshold_tens), [1, -1])[0]
    ...
    mesh = [tf.cast(f, 'float32')
            for f in volshape_to_meshgrid(windowsize, indexing='ij')]
    dist = tf.stack([mesh[f] - (windowsize[f] - 1) /
                    2 for f in range(len(windowsize))], axis=-1)
    ...
    kernel = tf.compat.v1.where(tf.less_equal(
        dist, dist_threshold_tens), tf.ones_like(dist), tf.zeros_like(dist))
...
def resample_tensor(tensor,
    ...
    if subsample_res is not None:
        ...
        assert len(subsample_res) == len(volume_res), 'subsample_res and volume_res must have the same length, ' \
                                                      'had {0}, and {1}'.format(
                                                          len(subsample_res), len(volume_res))
        if subsample_res != volume_res:

            # get shape at which we downsample
            downsample_shape = [
                int(tensor_shape[i] * volume_res[i] / subsample_res[i]) for i in range(n_dims)]

            # downsample volume
            tensor._keras_shape = tuple(tensor.get_shape().as_list())
            tensor = nrn_layers.Resize(
                size=downsample_shape, interp_method='nearest')(tensor)
    ...
    if resample_shape != downsample_shape:  # if we didn't downsample downsample_shape = tensor_shape
        tensor._keras_shape = tuple(tensor.get_shape().as_list())
        tensor = nrn_layers.Resize(
            size=resample_shape, interp_method=interp_method)(tensor)
    ...
    if build_reliability_map:
        ...
        if downsample_shape != tensor_shape:
            ...
            upsampling_factors = np.array(
                resample_shape) / np.array(downsample_shape)

            # build reliability map
            reliability_map = 1
            for i in range(n_dims):
                loc_float = np.arange(
                    0, resample_shape[i], upsampling_factors[i])
                ...
                reliability_map = reliability_map * \
                    np.reshape(tmp_reliability_map, shape)
